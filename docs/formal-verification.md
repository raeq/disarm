...
| `formal/lean/Text` | Lean 4 | case folding, the whitespace, control and invisible strips, zalgo, display width, punctuation, contractions and edit distance | #PR (a class-0 mark resetting the zalgo count; `strip_zalgo` output that was still zalgo; a Prepend character zeroing `terminal_width`; a stray VS16 widening a letter; `fold_punctuation` gaps; documentation claims) |
