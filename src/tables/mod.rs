...
/// True if ASCII byte `b` is a confusables→latin source the fold rewrites
/// (today `0x22`, `0x60`, `0x7C`). Backs the preset fast-path guard (#458); the
/// `[bool; 128]` table is generated by build.rs from the same TSV as the PHF.
pub(crate) fn is_ascii_confusable_latin(b: u8) -> bool {
    b < 0x80 && confusables_data::ASCII_CONFUSABLE_LATIN[b as usize]
}

/// Number of precomposed Hangul syllables (U+AC00–U+D7A3).
const HANGUL_SYLLABLE_COUNT: usize = 11_172;

// `HANGUL_BLOB` and `HANGUL_OFFSETS`: every precomposed syllable's romanization, packed
// into one string with an offset array (#237 item 3), generated by build.rs from
// `hangul::romanize_hangul` (codegen/hangul_table.rs). It was built on the first call into
// a `OnceLock`, which cost that call 3.5 million instructions, about three quarters of
// transliterating a 16 KiB Hangul document, and held two heap allocations for the life of
// the process; as statics it costs neither.
include!(concat!(env!("OUT_DIR"), "/hangul_table.rs"));
const _: () = assert!(HANGUL_OFFSETS.len() == HANGUL_SYLLABLE_COUNT + 1);
...
fn lookup_hangul_static(ch: char) -> Option<&'static str> {
    ...
    if (0xAC00..=0xD7A3).contains(&code) {
        ...
        // Flat indexed slice into the packed blob (#237 item 3). `HANGUL_OFFSETS` has
        ...
        let start = *HANGUL_OFFSETS.get(idx)? as usize;
        let end = *HANGUL_OFFSETS.get(idx + 1)? as usize;
        HANGUL_BLOB.get(start..end)
    } else {
...
pub fn match_emoji_sequence(window: &[char]) -> Option<(&'static str, usize)> {
    ...
    for (i, &c) in window.iter().enumerate() {
        ...
        match EDGE_CP[start..end].binary_search(&cp) {
            ...
            // against the fix).
            Err(_) if cp == VS16 && i >= 1 && selector_may_follow(window[i - 1]) => continue,
            Err(_) => break,
        }
