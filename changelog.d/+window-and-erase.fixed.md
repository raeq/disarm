- **`replace_emoji` cut emoji sequences at nine code points, leaving an invisible
  ...
  characters through a heap scan. 200,000 emoji: 4.91 ms against 4.71 ms before the
  change.
...
- **A backspace after a carriage return deleted the rest of the line (#995).**
  ...
  set, `"abc\rX\u{8}"` returned `""` where a terminal and a typed backspace both give
  `"bc"`, and a backspace at column 0 discarded the whole line. It now erases the one cell
  before the cursor and does nothing at column 0. Losing text the reader can see is the
  risk #934 declined to take.

  The cell is blanked rather than removed. A terminal moves the cursor on a backspace and
  leaves the screen alone; shifting is what a line editor does, and the two disagree on
  input as short as `"aa\ra\u{8}a"`, where shifting eats a character still on the screen.
