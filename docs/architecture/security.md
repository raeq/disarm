...
The `canonicalize` preset composes the security-relevant transforms into a single pipeline: resolve deletions → digit-policy pre-fold → NFKC normalization → strip bidi → strip invisibles → strip control → strip zero-width → whitespace collapse → drop repeated marks → strip zalgo → NFC → confusable normalization, iterated with NFC to a fixed point → drop repeated marks → strip zalgo. This is the recommended entry point for security-sensitive text processing where the goal is to canonicalize Unicode content by neutralizing homoglyph spoofing, removing dangerous bidi overrides, and collapsing invisible characters.

The `canonicalize_strict` preset extends this approach for web application input: resolve deletions → digit-policy pre-fold → NFKC normalization → strip bidi → strip zero-width → strip control → strip invisibles → confusable normalization (iterated with NFC and the cross-script mark strip to a fixed point) → drop repeated marks → strip zalgo (capping combining marks at 3 per base character, after the fold so a removed mark cannot split the count) → whitespace collapse → NFC, with no transliteration step. It is the recommended entry point for *normalizing* user-submitted form data, comments, and API payloads. Note: this is input Unicode normalization, not output sanitization — it provides no XSS/injection protection, so still encode at the output sink (see [Threat Model](https://github.com/raeq/disarm/blob/main/THREAT_MODEL.md)).
