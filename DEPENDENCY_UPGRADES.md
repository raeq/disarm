...
|--------------------|---------------------|
| **Build / codegen** (phf, phf_codegen) | `build.rs` compile-time assertions + full Rust suite, whose `src/phf_integrity.rs` checks every generated table's lookup and pinned content; the generated bytes may legitimately change |
