...
- **[Canonicalization pipelines](api/pipelines.md)**: `canonicalize`, `canonicalize_strict`, `catalog_key`, `search_key`, `sort_key`, `strip_format`, `ml_normalize` for common workflows
...
- **[Coverage introspection](user-guide/confusables.md#knowing-what-is-not-covered)**: `unmapped_confusables()` and `find_unmapped_confusables()` report which TR39 sources the bundled table does **not** fold — exposure you can measure against your own traffic, not a score
- **Bundled-data version**: `disarm.CONFUSABLES_VERSION` reports the `confusables.txt` release the tables were folded from, so a deployment can answer "am I stale?" without inferring it from behaviour ([provenance](provenance.md))
...
|---|---|---|
| `disarm.security` | Defense & safety analysis | `normalize_confusables`, `is_confusable`, `is_mixed_script`, `is_suspicious_hostname`, `strip_bidi`, `canonicalize` |
...
from disarm import is_confusable, canonicalize, decode_to_utf8, fold_case
```
