...
- **The scope above is also tested against named CVEs.** [`tests/test_cve_vectors.py`](https://github.com/raeq/disarm/blob/main/tests/test_cve_vectors.py)
  reconstructs the vector described in each of eleven published CVEs and asserts what
  disarm does with it — including the four it does **not** handle, whose negatives are
