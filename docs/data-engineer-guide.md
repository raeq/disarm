...
NFKC → confusables → strip_accents → transliterate → fold_case → collapse_whitespace.
...
|---|---|---|
| `catalog_key()` | NFKC → confusables → strip_accents → transliterate → fold_case → collapse_ws | Dedup keys, record matching |
