- **Fuzzing, coverage and mutation testing, all report-only.** A cargo-fuzz crate in
  ...
  properties that do not hold, left unfixed here: `slugify` drops up to 14 bytes of text
  after a numeric entity it cannot decode (`"Q&#A session"` gives `q`); the confusable
  ...
  asserted by the Python suite alone. `docs/architecture/testing-guarantees.md` has the
  reproductions and the baselines.
