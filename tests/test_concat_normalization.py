...
def unsafe_boundary(a: str, b: str) -> bool:
    ...
    The rule documented in `docs/RUST_API.md`. O(1), no tables, no disarm call.
    """
...
def test_the_documented_check_has_no_false_negatives() -> None:
    ...
    missed = []
    for _ in range(4000):
        a = "".join(random.choice(alphabet) for _ in range(random.randint(1, 3)))
        b = "".join(random.choice(alphabet) for _ in range(random.randint(1, 3)))
