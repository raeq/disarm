...
class EmojiDelimiterSegmentation(SuiteBase):
    ...
    provenance = Provenance(
        ...
        notes=(
            ...
            "U+1F22F, U+1F232..U+1F236, U+1F238..U+1F23A, U+1F250, U+1F251), "
            "whose compatibility decomposition is the bare ideograph, or, for "
            "U+1F201, the katakana `ココ` that `_runs` also counts as one "
            "alphanumeric run. U+1F237 lies between the two sub-ranges and is "
            "Emoji=Yes, Emoji_Presentation=No, so it never enters the domain. "
            "That mapping is normative. It is scored here because the "
            "*segmentation* consequence is real whatever its provenance: the "
            "word closes back up into one run around a character the reader "
            "did not write.\n\n"
