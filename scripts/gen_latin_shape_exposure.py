...
def render(rows: list[tuple[int, str, str]]) -> str:
    out = [
        ...
        "#",
        "# The UCD above is the HOST interpreter's, not disarm's — the selector reads",
        "# `unicodedata.name`, and CPython ships a different UCD per version. The gate",
        "# compares only code points the running interpreter can name, so a host with an",
        "# older UCD sees a subset and still agrees. See tests/test_latin_shape_exposure.py.",
        "#",
