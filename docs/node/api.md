...
`searchKey` is a case/accent/script-insensitive search key; `sortKey` is the same
but keeps base accented characters so collation orders correctly; `catalogKey`
