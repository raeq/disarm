...
def write_digit_tr39_overrides(
    ...
    disarm maps a non-Latin digit to the ASCII digit; TR39 maps several of them to a Latin
    *letter* (Devanagari zero to ``o``, Kannada zero to ``O``, Arabic-Indic one to ``l``).
    ...
    with open(path, "w", encoding="utf-8") as f:
        f.write(
            ...
            f"TR39 folds it to a letter; the value here is TR39's target, applied only "
            f"under digit_policy='tr39'. Generated by scripts/gen_confusables.py.\n"
        )
