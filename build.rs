...
const COMMON_SCRIPT_TARGETS: &[char] = &[
...
/// at a stale value.
fn read_confusables_version(path: &Path) -> String {
    ...
    let rest = header.strip_prefix(MARKER).unwrap_or_else(|| {
        panic!(
            "{}: header line does not start with {MARKER:?} (got {header:?}). \
             ...
             `read_confusables_version` in build.rs in the same commit.",
            path.display()
        )
    });
...
/// hand-regenerated.
fn generate_decimal_digit_zeros(path: &Path, out: &Path) {
    let content = fs::read_to_string(path)
        .unwrap_or_else(|e| panic!("Failed to read {}: {e}", path.display()));
