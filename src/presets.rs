...
pub fn _ml_normalize(text: &str, lang: Option<&str>, emoji_style: &str) -> PyResult<String> {
    ...
    if lang.is_some() {
        buf = transliterate::transliterate_impl(&buf, lang, crate::ErrorMode::Preserve, "", false, false)
            .into_owned();
    }
...
pub fn _catalog_key(text: &str, lang: Option<&str>, strict_iso9: bool) -> PyResult<String> {
    ...
    let buf = if lang.is_some() || strict_iso9 {
        transliterate::transliterate_impl(&buf, lang, crate::ErrorMode::Preserve, "", strict_iso9, false)
            .into_owned()
    } else {
        buf
    };
